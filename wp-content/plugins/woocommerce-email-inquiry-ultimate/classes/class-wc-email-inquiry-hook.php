...
class WC_Email_Inquiry_Hook_Filter
{
	...
	public static function add_email_inquiry_button($product_id, $page_type= 'single') {
		global $post;
		global $wc_email_inquiry_contact_form_settings, $wc_email_inquiry_3rd_contact_form_settings;
		...
		if ( WC_Email_Inquiry_3RD_ContactForm_Functions::check_enable_3rd_contact_form() ) {
			...
			if ( $page_type == 'single' ) {
				$open_type = $wc_email_inquiry_3rd_contact_form_settings['product_page_open_form_type'];
				...
				$open_type = $wc_email_inquiry_3rd_contact_form_settings['category_page_open_form_type'];
				if ( $open_type == 'popup' ) {
					$email_inquiry_page_link = '';
					$email_inquiry_button_class = 'wc_email_inquiry_button_3rd';
				} elseif ( $open_type == 'new_page_same_window' ) {
...
<div style="padding:10px;">
	...
	public static function wc_email_inquiry_popup_3rd() {
		check_ajax_referer( 'wc_email_inquiry_popup_3rd', 'security' );
	...
	public static function footer_print_scripts() {
		...
		if ( WC_Email_Inquiry_3RD_ContactForm_Functions::check_enable_3rd_contact_form() && $wc_email_inquiry_3rd_contact_form_settings['product_page_open_form_type'] != 'popup' && $wc_email_inquiry_3rd_contact_form_settings['category_page_open_form_type'] != 'popup' ) return;
	...
	public static function script_contact_popup() {
		...
		if ( WC_Email_Inquiry_3RD_ContactForm_Functions::check_enable_3rd_contact_form() ) {
			if (get_option('permalink_structure') == '')
				$email_inquiry_page_link = get_permalink( $wc_email_inquiry_page_id ).'&product-id='.$product_id.'" ';
			else
				$email_inquiry_page_link = 'href="'.rtrim( get_permalink( $wc_email_inquiry_page_id ), '/' ).'/product-id/'.$product_id.'" ';
	?>
...
(function($){
	$(function(){
    <?php if ( $wc_email_inquiry_3rd_contact_form_settings['product_page_open_form_type'] == 'inner_page' ) { ?>
...
jQuery(document).ready(function() {
	...
	public static function plugin_extra_links($links, $plugin_name) {
		...
		$links[] = '<a href="http://docs.a3rev.com/user-guides/plugins-extensions/woocommerce/email-inquiry-ultimate/" target="_blank">'.__('Documentation', 'wc_email_inquiry').'</a>';
		$links[] = '<a href="https://a3rev.com/forums/forum/woocommerce-plugins/email-inquiry-ultimate/" target="_blank">'.__('Support', 'wc_email_inquiry').'</a>';
		return $links;
	}
}
