...
class WC_Email_Inquiry_Upgrade
{
	...
	public static function get_version_info($cache=true){
		...
		$respone_api = get_transient("a3rev_wc_email_inquiry_ultimate_update_info");
		if(!$cache)
            $respone_api = null;
		...
		if(!$respone_api){
				$options = array(
					...
					'body' 		=> array(
									...
									'key'			=> get_option('a3rev_auth_wc_email_inquiry_ultimate'),
									'domain_name'	=> $_SERVER['SERVER_NAME'],
									'address_ip'	=> $_SERVER['SERVER_ADDR'],
									'v'				=> get_option('a3rev_wc_email_inquiry_ultimate_version'),
			...
			//caching responses.
			set_transient("a3rev_wc_email_inquiry_ultimate_update_info", $respone_api, 86400); //caching for 24 hours
		}
	...
	public static function check_update($update_plugins_option){
        ...
        if($new_version['is_valid_key'] != 'valid' || version_compare(get_option('a3rev_wc_email_inquiry_ultimate_version'), $new_version['version'], '>=')){
            unset($update_plugins_option->response[$plugin_name]);
        }else{
            $update_plugins_option->response[$plugin_name]->url = "http://www.a3rev.com";
            $update_plugins_option->response[$plugin_name]->slug = get_option('a3rev_wc_email_inquiry_ultimate_plugin');
	...
	//Displays current version details on Plugin's page
   	public static function display_changelog(){
        if($_REQUEST["plugin"] != get_option('a3rev_wc_email_inquiry_ultimate_plugin'))
            return;
	...
	public static function get_url_download(){
        $download_url = WC_EMAIL_INQUIRY_MANAGER_URL . "/download.php?plugin=".get_option('a3rev_wc_email_inquiry_ultimate_plugin')."&key=".get_option('a3rev_auth_wc_email_inquiry_ultimate')."&domain_name=".$_SERVER['SERVER_NAME']."&address_ip=" . $_SERVER['SERVER_ADDR']."&v=".get_option('a3rev_wc_email_inquiry_ultimate_version')."&owner=".base64_encode(get_bloginfo('admin_email'));
	...
	public static function make_compatibility( $info, $action, $args ) {
		...
		$our_plugin_name = get_option('a3rev_wc_email_inquiry_ultimate_plugin');
